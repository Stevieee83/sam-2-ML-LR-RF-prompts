...
#     """
#     # Load mask in palette mode (P) to preserve class indices
...
#     if mask_prompt.shape != image_np.shape[:2]:
#         mask_prompt_img = Image.fromarray((mask_prompt * 255).astype(np.uint8))
...
#
#     # Prepare mask input for SAM 2
#     mask_input = mask_prompt[None, :, :]  # Add batch dimension: (1, H, W)
#
...
#         multimask_output=True,  # Returns 3 masks with different quality-coverage tradeoffs
#     )
#
...
#     image_path = "C:/Users/r02sw23/Documents/borebreen-drone-image-data/binary_masks/borebreen_crop_drone_1.png"
#     voc_mask_path = "C:/Users/r02sw23/Documents/borebreen-drone-image-data/images/borebreen_crop_drone_1.png"
...
#     result_mask.save("output_refined_mask.png")
#
#     # Optionally: Save all masks
#     for i, (mask, score) in enumerate(zip(masks, scores)):
#         mask_img = Image.fromarray((mask * 255).astype(np.uint8))
#         mask_img.save(f"output_mask_{i}_score_{score:.3f}.png")
#######################################################################################################################
# import torch
# import numpy as np
# from PIL import Image
...
#         mask_prompt_img = mask_prompt_img.resize(
#             (image_np.shape[1], image_np.shape[0]),
#             Image.Resampling.NEAREST  # Fixed deprecation warning
#         )
#         mask_prompt = np.array(mask_prompt_img).astype(np.float32) / 255.0
#
#     # Convert mask to torch tensor with correct shape [1, 1, H, W]
#     mask_input = torch.from_numpy(mask_prompt).unsqueeze(0).unsqueeze(0)  # Add batch and channel dims
#
...
#     mask_prompt = load_pascal_voc_mask(mask_path, class_id)
#
#     print(f"Initial mask shape: {mask_prompt.shape}")
...
#         mask_prompt = np.array(mask_prompt_img).astype(np.float32) / 255.0
#
#     print(f"Resized mask shape: {mask_prompt.shape}")
#
#     # Ensure mask is 2D
#     if len(mask_prompt.shape) == 3:
#         mask_prompt = mask_prompt[:, :, 0]
#
#     print(f"Final mask shape before tensor conversion: {mask_prompt.shape}")
#
#     # Convert mask to torch tensor with correct shape [1, 1, H, W]
#     mask_input = torch.from_numpy(mask_prompt).float()
#     mask_input = mask_input.unsqueeze(0).unsqueeze(0)  # Add batch and channel dims
#
#     print(f"Mask tensor shape: {mask_input.shape}")
#
#     # Move to same device as model
#     device = next(predictor.model.parameters()).device
#     mask_input = mask_input.to(device)
...
if __name__ == "__main__":
    # Paths
    checkpoint = 'C:/model_weights/sam-2/sam2_hiera_base_plus.pt'
    image_path = "C:/Users/r02sw23/Documents/borebreen-drone-image-data/masks/borebreen_crop_drone_1.png"
    #voc_mask_path = "C:/Users/r02sw23/Documents/borebreen-drone-image-data/images/borebreen_crop_drone_1.png"
